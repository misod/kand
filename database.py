# database related stuff
import logging
import pymysql

def login():

    return ""

def logout():

    return ""

def get_plane_ids():
    # testfunk for returning relevant plane ids
    return [0x484B46, 0xDDA68F, 0xDDA477, 0xDDD86A, 0xDDA3F3, 0xDF0EE6, 0xDDE02C, 0x3E5C99, 0xDDEEE3]

def get_started_flight():

    return ""

def new_flight():

    return ""

def get_airfields_height():
    # the height above sealevel for parked planes
    # mabye set this as a variable that the users define when setting up the system
    # or let the program determine this and se how it behaves

    return ""
